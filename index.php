...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($username === '' || $password === '') {
        ...
        if (!$stmt) {
            ...
            if (!$user) {
                // Auto-create staff user if it doesn't exist
                if ($username === 'staff' && $password === '123') {
                    $newHash = password_hash($password, PASSWORD_DEFAULT);
                    $insert = $conn->prepare("INSERT INTO users (username, password_hash, role) VALUES (?, ?, 'staff')");
                    if ($insert) {
                        ...
                        if ($newId > 0) {
                            ...
                            $stmt2 = $conn->prepare("SELECT id, username, password_hash, role, COALESCE(totp_enabled, 0) AS totp_enabled, COALESCE(totp_secret, '') AS totp_secret FROM users WHERE id = ? LIMIT 1");
                ...
                if (!$ok && $username === 'staff' && $password === '123') {
                    $newHash = password_hash($password, PASSWORD_DEFAULT);
                    $u = $conn->prepare("UPDATE users SET password_hash = ? WHERE id = ?");
                    if ($u) {
                        $uid = (int)$user['id'];
