...
if (isset($conn) && $conn instanceof mysqli && !$conn->connect_error) {
    ...
    // Check for pending products (staff only)
    if ($res = $conn->query("SHOW TABLES LIKE 'pending_products'")) {
        if ($res->fetch_assoc()) {
            ...
            if ($isStaff && $userId > 0) {
                $stmt = $conn->prepare("SELECT COUNT(*) AS c FROM pending_products WHERE requested_by = ?");
                if ($stmt) {
                    $stmt->bind_param('i', $userId);
                    $stmt->execute();
                    $res = $stmt->get_result();
                    if ($row = $res->fetch_assoc()) {
                        $stats['pending_approval'] = (int)($row['c'] ?? 0);
                    }
                    $stmt->close();
                }
            }
        ...
        <section class="home">
            ...
            <div class="kpi-grid">
                <div class="kpi-card">
                    ...
                    <div class="kpi-value" id="products-count"><?php echo (int)$stats['products']; ?></div>
                    <div class="kpi-icon bg-blue"><i class='bx bxl-product-hunt'></i></div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-label">Categories</div>
                    <div class="kpi-value" id="categories-count"><?php echo (int)$stats['categories']; ?></div>
                    <div class="kpi-icon bg-green"><i class='bx bxs-category-alt'></i></div>
                </div>
                <div class="kpi-card">
                    <div class="kpi-label">Low Stock</div>
                    <div class="kpi-value" id="low-stock-count"><?php echo (int)$stats['low_stock']; ?></div>
                    <div class="kpi-icon bg-purple"><i class='bx bx-error-circle'></i></div>
                </div>
                <?php $isStaff = ($_SESSION['role'] ?? '') !== 'admin'; ?>
                <?php if ($isStaff) { ?>
                <div class="kpi-card">
                    <div class="kpi-label">Pending Approval</div>
                    <div class="kpi-value" id="pending-count"><?php echo (int)$stats['pending_approval']; ?></div>
                    <div class="kpi-icon bg-orange"><i class='bx bx-time-five'></i></div>
                </div>
                <?php } else { ?>
                <div class="kpi-card">
                    <div class="kpi-label">Stock Movements</div>
                    <div class="kpi-value" id="movements-count"><?php echo (int)$stats['movements']; ?></div>
